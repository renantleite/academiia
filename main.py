...
async def buscar_treino_por_data(usuario: str, data_busca: str):
    ...
    inicio_dia = data_inicio.replace(hour=0, minute=0, second=0, microsecond=0)
    fim_dia = data_inicio.replace(hour=23, minute=59, second=59, microsecond=999999)

    cursor = colecao_treinos.find({
        "usuario": usuario,
        "data_treino": {"$gte": inicio_dia, "$lte": fim_dia}
    }).sort("data_treino", 1)
